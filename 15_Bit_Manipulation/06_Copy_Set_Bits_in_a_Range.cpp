...
// We can one by one find set bits of y by traversing given range. For every set bit, we OR it to existing bit of x, so that the becomes set in x,
// if it was not set. Below is C++ implementation.

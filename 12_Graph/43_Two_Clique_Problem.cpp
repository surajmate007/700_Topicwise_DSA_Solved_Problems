// A Clique is a subgraph of graph such that all vertices in subgraph are completely connected with each other. Given a Graph, find if it can be divided
// into two Cliques.
// This problem looks tricky at first, but has a simple and interesting solution. A graph can be divided in two cliques if its complement graph is
// Bipartitite. So below are two steps to find if graph can be divided in two Cliques or not. 
// Find the complement of Graph. Below is the complement graph is above shown graph. In complement, all original edges are removed. And the vertices
// which did not have an edge between them, now have an edge connecting them. 
